...
cusolverStatus_t CUSOLVERAPI cusolverSpXcsrcholDiag(
						    cusolverSpHandle_t handle,
						    csrcholInfo_t      info,
						    double *           diag)
{
  // cusolverSp<t>csrcholDiag must be called after cusolverSp<t>csrcholFactor otherwise "diag" is wrong.
  return 
    cusolverSpXcsrcholDiag(
			   handle,
			   info,
			   diag);
}

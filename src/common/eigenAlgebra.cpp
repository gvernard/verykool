...
StandardAlgebra::StandardAlgebra(BaseLikelihoodModel* a){
  this->likeModel = a;
}
...
//===============================================================================================================
PerturbationsAlgebra::PerturbationsAlgebra(BaseLikelihoodModel* a){
  this->likeModel = a;
}

void PerturbationsAlgebra::setAlgebraInitPert(ImagePlane* image,Pert* pert_mass_model,double* res){
  ...
  if( pert_mass_model->dpsi->reg == "covariance_kernel" ){
    // calculate the inverse of the perturbations covariance matrix stored in H, and store it in HtH
...
void PerturbationsAlgebra::setAlgebraRuntimePert(ImagePlane* image,BaseSourcePlane* source,Pert* pert_mass_model,BaseLikelihoodModel* smooth_like,double lambda){
  Eigen::SparseMatrix<double> Ds(source->Ds.Ti,source->Ds.Tj);
  Ds.reserve(Eigen::VectorXi::Constant(source->Ds.Tj,2));//setting the non-zero coefficients per row of the derivative matrix for the source (2 for first order gradient, etc)
  for(int i=0;i<source->Ds.tri.size();i++){  Ds.insert(source->Ds.tri[i].i,source->Ds.tri[i].j) = source->Ds.tri[i].v;  }//for column-major (swap the i and j indices of tri struct)}

  StandardLikelihood* like_pointer = dynamic_cast<StandardLikelihood*>(smooth_like);
  ...
  M_pert = like_pointer->algebra->B * Ds * this->Dpsi;
  Mt_pert = M_pert.transpose();
  
  this->M_pert  = M_pert;
  this->Mt_pert = Mt_pert;

  Eigen::SparseMatrix<double> A_pert(image->Nm,pert_mass_model->dpsi->Sm);
  if( pert_mass_model->dpsi->reg == "covariance_kernel" ){
    A_pert = Mt_pert*like_pointer->algebra->C*M_pert + this->HtH_pert;
  } else {
    A_pert = Mt_pert*like_pointer->algebra->C*M_pert + lambda*this->HtH_pert;
  }
  this->A_pert = A_pert;
...
void PerturbationsAlgebra::solvePert(ImagePlane* image,BaseSourcePlane* dpsi,BaseLikelihoodModel* smooth_like){
  int Nm = image->Nm;
  int Sm = dpsi->Sm;
  ...
  //Get the Most Probable solution for the source
  Eigen::VectorXd new_dpsi(Sm);
  StandardLikelihood* like_pointer = dynamic_cast<StandardLikelihood*>(smooth_like);
  new_dpsi = inv*(this->Mt_pert*like_pointer->algebra->C*this->dd);
  Eigen::Map<Eigen::VectorXd>(dpsi->src,new_dpsi.size()) = new_dpsi;

  //  for(int i=0;i<image->Nm;i++){
  //    std::cout << this->dd[i] << std::endl;
  //  }
